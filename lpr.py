...
from data_list import overSpeedArr, licenseDataArr
...
def recognizeLic(probLicImages):
    ...
    for licImg in probLicImages:
        licNoTemp = pytesseract.image_to_string(licImg, config=config).strip()
        if licNoTemp:
...
def lprMain(imgsrc):
    ...
    cv2.imshow('Orignal', imutils.resize(original, height=360)) #uncomment to show car image
...
#function -> will be called from main.py; executed in independent thread
def lprLooped():
    frameTime = None
    speed = None
    imgsrc = None
    ...
    while True:
        if len(overSpeedArr) > 0:
            print('\n---------- Thread 2 ----------')
            frameTime, speed, imgsrc = overSpeedArr.pop(0)
            finalLicNo = lprMain(imgsrc)

            licenseDataArr.append((frameTime, speed, finalLicNo, imgsrc))

            print('Thread 2 : License Number : {}'.format(finalLicNo))

            cv2.waitKey(0) #uncomment to show car image
            cv2.destroyAllWindows() #uncomment to show car image
...
if __name__ == '__main__':
    ...
    cv2.imshow('Orignal', imutils.resize(original, height=360))
    cv2.waitKey(0)
    cv2.destroyAllWindows()
